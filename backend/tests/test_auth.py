...
def test_login_sets_private_cookies_initializes_owned_rows_and_returns_only_identity(
    ...
    assert response.json() == {
        "user": {"id": provider.user_id, "email": provider.email},
        "expires_at": provider.session_end,
    }
...
def test_every_write_requires_csrf(
    account_client: TestClient, provider: ProviderFixture, path: str
) -> None:
...
def test_validation_errors_never_echo_inputs(
    account_client: TestClient, body: dict[str, object]
) -> None:
...
def test_signup_and_resend_return_generic_messages(
    ...
    response = account_client.post(
        "/auth/resend-verification",
        headers=headers(account_client),
        json={"email": "account@example.com"},
    )
    assert response.status_code == 202
    assert "existing" not in response.text


def test_email_code_verification_uses_email_type(
    account_client: TestClient, provider: ProviderFixture
) -> None:
    response = account_client.post(
        "/auth/verify-email",
    ...
    assert json.loads(request.content)["type"] == "email"
...
def test_profile_and_settings_are_owned_allowlisted_and_real_transport_updates(
    ...
    response = account_client.patch(
        "/profile", headers=headers(account_client), json={"display_name": "  Test user  "}
    )
    assert response.status_code == 200
...
@pytest.mark.parametrize(
    ...
    [
        ("/profile", {"id": "forged"}),
        ("/profile", {"created_at": "2026-01-01"}),
        ("/profile", {"display_name": "x" * 81}),
        ("/profile", {}),
        ("/settings", {"preferred_language": "fr"}),
...
def test_refresh_replay_is_bounded_and_logout_invalidates_cached_refresh(
    ...
    old_access, old_refresh = (
        account_client.cookies["sl_access"],
        account_client.cookies["sl_refresh"],
    )
...
def test_expired_session_cannot_be_revived_by_refresh(
    account_client: TestClient, provider: ProviderFixture
) -> None:
    assert login(account_client).status_code == 200
...
def test_provider_failure_preserves_session_but_logout_clears_it_honestly(
    ...
    assert login(account_client).status_code == 200
    provider.failures["/rest/v1/rpc/session_context"] = (
        500,
...
def test_auth_attempt_rate_limit(account_client: TestClient, provider: ProviderFixture) -> None:
    ...
    for _ in range(3):
        assert (
            account_client.post(
                "/auth/resend-verification", json={"email": "account@example.com"}, headers=values
            ).status_code
            == 202
        )
    response = account_client.post(
...
def test_oversized_request_rejected_before_provider_io(
    account_client: TestClient, provider: ProviderFixture
) -> None:
    response = account_client.post(
        "/auth/login", headers=headers(account_client), json={"email": "x" * 17000}
    )
