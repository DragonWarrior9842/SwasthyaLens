...
class AccountsRepository:
    ...
    def session_expiry(self, token: str) -> int:
        context = self.gateway.object(
            "POST", "/rest/v1/rpc/session_context", payload={}, access_token=token,
        )
    ...
    def ensure_rows(self, identity: VerifiedIdentity, token: str) -> None:
        for table, column in (("profiles", "id"), ("user_settings", "user_id")):
            self.gateway.request(
                "POST", f"/rest/v1/{table}", payload={column: str(identity.user_id)},
                access_token=token, params={"on_conflict": column},
                prefer="resolution=ignore-duplicates,return=minimal",
            )

    def _row(
        self, table: str, column: str, columns: str, identity: VerifiedIdentity, token: str,
        changes: dict[str, object] | None,
    ) -> object:
        result = self.gateway.request(
            "GET" if changes is None else "PATCH", f"/rest/v1/{table}",
            payload=changes, access_token=token,
    ...
    def profile(
        self, identity: VerifiedIdentity, token: str, changes: dict[str, object] | None = None,
    ) -> Profile:
        row = self._row(
            "profiles", "id", "id,display_name,created_at,updated_at", identity, token, changes,
        )
        try:
    ...
    def settings(
        ...
        row = self._row(
            "user_settings", "user_id", "user_id,preferred_language,timezone,created_at,updated_at",
            identity, token, changes,
        )
        try:
