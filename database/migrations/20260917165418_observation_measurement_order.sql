-- Prefer actual manual instants within each measurement day; day-only records have no invented time.
begin;
...
begin
 if p_operation='publish' then
  ...
  if report is not null and not exists(select 1 from public.reports where id=report and user_id=caller and status='uploaded') then
   raise exception using errcode='P0001',message='observation_not_found'; end if;
  select coalesce(jsonb_agg(swasthyalens_private.observation_document(q.id,false) order by q.day desc nulls last,q.measured_at desc nulls last,q.created_at desc,q.id desc),'[]'::jsonb) into items from (
   select h.id,h.created_at,latest_row.measured_at,coalesce(latest_row.measurement_date,(latest_row.measured_at at time zone 'UTC')::date) as day
   ...
   order by day desc nulls last,latest_row.measured_at desc nulls last,h.created_at desc,h.id desc limit 21 offset offset_rows
  ) q;
